...
import { ActivityIndicator, View, Text, RefreshControl, StyleSheet, TextInput } from 'react-native';
import { useFetchProductsQuery, useRateProductMutation, useSaveProductMutation, useSearchProductsQuery } from '../../../redux/features/product/productApi';
...
const ProductScreen = ({ navigation }) => {
  ...
  const [modalContent, setModalContent] = useState({ icon: '', message: '', buttonColor: Colors.Green });
  const [selectedId, setSelectedId] = useState(null);

  const { data, isLoading, isFetching, refetch } = useFetchProductsQuery({ page: currentPage });

  console.log("data", data)
  const { data: searchData, error: searchError, isLoading: isSearching } = useSearchProductsQuery(selectedId || searchText || "");
  ...
  useEffect(() => {
    if (searchText || selectedId) {
      ...
      setProducts(prevProducts => {
        const newProducts = data.data.filter(product => !prevProducts.some(p => p.id === product.id));
        return currentPage === 1 ? newProducts : [...prevProducts, ...newProducts];
      });
      setHasMore(currentPage < (data.total_pages || 0));
    }
  }, [data, searchData, searchText, selectedId, currentPage]);
  ...
  console.log("products", products.lengt)

  const [saveProduct] = useSaveProductMutation();
  const [rateProduct] = useRateProductMutation();

  const handleSaveProduct = async (productId) => {
    const isProductSaved = savedProducts.some(product => product.id === productId);
  ...
  return (
    <MainContainer>
      <Header 
        ...
        Press={() => navigation.navigate("Profile", { screen: ROUTES.PROFILE_SCREEN })}
      />

     <FlatList
        ListHeaderComponent={
          <>
            <Text style={styles.title}>{translate('What would you like to order')}</Text>
            <View style={styles.searchContainer}>
              <Ionicons name="search" size={25} color={Colors.Orange} style={styles.searchIcon} />
              <TextInput
