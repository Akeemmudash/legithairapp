...
const CheckOutScreen = ({ route, navigation }) => {
  ...
  const { full_name, phone, id, email } = useSelector(
  ...
  useEffect(() => {
    if (selectedCountry) {
      ...
      axios
        .post(STATES_API_URL, { country: selectedCountry })
        .then((response) => {
  ...
  const handleOrderCreation = async () => {
    ...
    const orderDetails = {
      products: items.map((item) => {
        const hasDiscount =
          item.selectedInch?.discount && item.selectedInch.discount > 0;

        const productAmount = item.selectedInch
          ? hasDiscount
            ? item.selectedInch.discount
            : item.selectedInch.price
          : item.price;
        const initialAmount = item.selectedInch
          ? hasDiscount
            ? item.selectedInch.price
            : 0
          : 0;
  ...
  const handleOnRedirect = async (data) => {
    ...
    try {
      ...
      console.error(
        "Error during transaction verification or order creation:",
        error,
      );
      setModalContent({
        icon: "close-circle-outline",
      ...
      <FlatList
        ...
        ListFooterComponent={
          <>
                  ...
                  onPress={() => {
                    if (selectedPlace === "pickup" && !checkedItems) {
                      setModalContent({
                        icon: "close-circle-outline",
                        message:
                          "Please select a pickup center before proceeding.",
                        buttonColor: Colors.Orange,
                        iconColor: Colors.Orange,
                      });
                      setModalAlertVisible(true);
                    ...
                    if (
                      ...
                      setModalContent({
                        icon: "close-circle-outline",
                        message:
                          "Provide a deliver LandMark before proceeding.",
                        buttonColor: Colors.Orange,
                        iconColor: Colors.Orange,
                      });
                      setModalAlertVisible(true);
            ...
            {selectedOption === "bank" && (
              <CustomButton
                title={isLoading ? "Creating Order..." : "Send Receipt"}
                ...
                onPress={() => {
                  ...
                  if (
                    ...
                    setModalContent({
                      icon: "close-circle-outline",
                      message:
                        "Please provide a delivery address before proceeding.",
                      buttonColor: Colors.Orange,
                      iconColor: Colors.Orange,
                    });
                    setModalAlertVisible(true);
